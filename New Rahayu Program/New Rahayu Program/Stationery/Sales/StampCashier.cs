...
{
    public partial class StampCashier : Form
    {
        MainForm main;

        public StampCashier(MainForm main)
        {
            this.main = main;
            InitializeComponent();
        }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (stampEmployee != 0)
            {
                ...
                DataTable dt = ExecuteQuery("SELECT SUM((sellPrice * quantity) - discount) AS totalBelanja FROM StationerySalesDetail ssd WHERE ssd.stationerySalesID IN (SELECT stationerySalesID FROM StationerySalesHeader ssh WHERE salesTime > (SELECT stampTime FROM StationeryTimeStamp WHERE description = 'CLOSING HARIAN' ORDER BY stampTime DESC LIMIT 0, 1))");
                int totalBelanja = 0;
                ...
                ExecuteQuery("INSERT INTO StationeryTimeStamp (stampTime, employeeID, description, cashDrawer, totalPenjualan) VALUES (now(), " + stampEmployee + ", 'HITUNG SELISIH', '" + total + "', '" + totalBelanja + "')");
